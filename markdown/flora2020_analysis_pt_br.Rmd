...
Essa é uma pequena análise explorando os dados de espécies nativas da flora brasileira. Esta é parte de uma série de três análises integrando dados do projeto Flora 2020 do Jardim Botânico do Rio de Janeiro, Global Biodiversity Information Facility (GBIF) e a Lista Vermelha da IUCN. O objetivo dessa série é analisar algumas informações jogadas em minha nuvem de "coisas para explorar da flora" que não tive tempo enquanto ainda trabalhando na área, ao passo que treino/refino algumas técnicas de análise de dados e estatística em geral e ainda guardo alguns códigos, paradigmas e pipelines úteis para outras instâncias.

Esta primeira parte visa extrair, manipular e limpar dados da flora 2020 e constuir algumas visualizações gerais sobre os dados. Uma segunda parte, mais voltada para a análise estatística dos dados e desenho experimental já está em andamento e uma terceira já está planejada para ser conduzida em seguida. Todas estas análises estão sob constante revisão, sofrendo constantes alterações

Está análise está dividida em duas seções:

1. **Extração, limpeza e compilação de dados:** Aqui extraio os dados do projeto flora 2020 direto do IPT do JBRJ, observo a estrutura dos dados e formato/limpo ele da maneira que preciso para analisa-los da forma que pretendo. Essa parte da análise é relativamente extensa e técnica, sem análises de dados ou visualizações. Algumas das coisas dessa seção: especificamente aqui:
    + Extração e descompressão dos dados da flora 2020 
...
2. **Análise e visualização de dados:** Aqui, analiso o data frame unificado criado na etapa anterior, ploto algumas visuzalizações e mato a curiosidade sobre alguns aspectos dos dados. Algumas das coisas dessa seção:
...
* [`lubridate`](https://lubridate.tidyverse.org/): Manipulação de data e hora.
...
* [`PNWColors`](https://github.com/jakelawlor/PNWColors): Uma paleta de cores que gostei.
* [`ggExtra`]()
* [`plotly`]()
* [`cowplot`]()
...
Os dados utilizados se referem àqueles gerados pelo Projeto Flora 2020 do JBRJ. Um dos grandes objetivos desse projeto é catalogar uma série de dados associados com as espécies da flora brasileira descrita até então, no âmbito da "Estratégia Global para Conservação de Plantas (GSPC)". Mais sobre esse projeto pode ser visto aqui. Dentre os dados disponíveis, quero compilar para analisar aqueles pertencentes à quatro grandes grupos:
...
```{r}
# primeiro as funções já citadas:
library(tidyverse) 
library(lubridate)
library(patchwork) 
library(ggrepel) 
...
library(plotly)
library(cowplot)
# depois as funções que eu escrevi
...
1. **taxon.txt:** contém as informações mais básicas que preciso para começar a análise, como as informações taxonômicas das entradas e autores de descrição. Lembrando que os dados desse arquivo se aplicam a **todos** níveis taxonômicos contemplados no projeto (**e.g.** reino e gênero).

2. **reference.txt**: contém informações relativas à descrição dos taxa (**e.g.** ano e autor).

3.**distribution.txt:** informa se os taxa são endemicos ou não e em que domínios fitogeográficos ou estados brasileiros eles ocorrem. 
...
Finalmente, a tabela `taxonRank` indica o nível taxonomico do registro:
...
```{r}
taxon <- taxon %>% filter(taxonomicStatus == "NOME_ACEITO", # apenas nomes aceitos
...
Parece razoável, embora ainda vá mexer nos tipos das variáveis mais adiante. A tabela contém a informação de `r dim(df_master)[1]` espécies nativas da flora brasileira, mas ainda falta coisa para ser compilada.

## Compilando os dados restantes
***

Agora vou extrair as demais informações seguindo uma lógica utilizada na tabela `taxon`, mas usando a função `match`. Especificamente, quero puxar os dados abaixo das espécies:

* Ano de descrição
* Autor da descrição
* Domínios fitogeográficos de ocorrência
* Estados da Federação (ou do que resta dela) de ocorrência
* Formas de vida
...
O mínimo e o máximo ainda estão bem esquisitos, vou manter apenas datas que se encontram em um intervalo de tempo rezoável: entre 1700 (pouco antes do estabelecimento do sistema lineniano) e 2020. O resto vou converter para NA.
...
```{r}
# nessa função, indico qual o separador que vou usar depois de especificar uma id (x), coluna e tabela.
...
Vou extrair nesse formato mesmo e fazer o parse mais adiante:

```{r}
df_master['distribution']<-distribution$occurrenceRemarks[match(ids_master,distribution$id)]
```

As informações ecológicas estão armazenadas de maneira semelhante, na tabela `profile`, em uma coluna de nome não muito intuitivo: `lifeForm`. Incluir esses dados direto em uma coluna do `df_master` e fazer o parsing depois.
...
A maior parte dos dados que preciso já estão no `df_master`, mas ainda preciso fazer o parsing daquelas colunas no formato json. Vou chamar uma função que fiz para lidar com isso, o que essa função faz é quebrar em colunas a informação contida nos json, por exemplo, isso aqui:
...
Toma esse formato, onde as chaves se tornam colunas com a informação pertinente:
...
df_master <- json_parser(df_master$profile) %>% 
              bind_cols(df_master) %>% 
              select(-profile,-habitat) # alem de remover a coluna obsoleta, tirei a coluna da chave habitat, que não vai ser utilizada na análise
```
...
head(df_master)
```
Ainda me resta extrair apenas o que quero da coluna `higherClassification` que contém infomrações taxonômicas das espécies, que está formatada dessa maneira:
...
df_master <- df_master %>% 
        ...
        mutate_at(c("lifeForm","endemism","vernacular_name","major_group"),
                  iconv,from="UTF-8",to="ASCII//TRANSLIT") # iconv remove os acentos dessas colunas 
...
A maior parte das colunas está formatada como string, embora a maioria seja constituída de variáveis categóricas, vou converter todas que não contém separadores e que não são números para fatores:
...
As variáveis que quero investigar podem ser agrupadas em 5 grupos e vou analisá-las de agregando informações de grupos distintos:
...
# criando rapidamente tabela para o plot
  # contando ocorrência dos anos
  # incluindo uma coluna com o século de cada ano
...
geom_text(aes(x = 1920, y = 750, label = "855 espécies \ndescritas em 2001"), 
             ...
             label.size = NA, 
             size = 4) +
  geom_curve(aes(x = 2001, y = 855, xend = 1980, yend = 800), # acho que a seta não saiu no plot por causa do cowplot
                             colour = "#555555", 
                             size=0.5, 
                             curvature = 0.2,
  ...
  theme(legend.title = element_blank()) + ggtitle('Número de descrições \nanuais de espécies') +
  scale_y_continuous(limits = c(0,800),breaks=seq(0,800,150))


### Aqui vale uns labels no formato de tabela com os n de cada grupo
...
p2 <- df_master %>% 
  ...
  theme_bw() +
  removeGridX() +
  theme(panel.border = element_blank(), legend.position = "none") + xlab("") +
  scale_y_continuous(limits = c(0,800),breaks=seq(0,800,150))
...
(grid_p <- plot_grid(p1,p2,p3, nrow=2,ncol = 2,labels = c('A', 'B','C')))
```

A curva indica um aumento na taxa de descrições ao longo do século 18, seguida por leve diminuição na taxa em meados do século 19, que finalmente, assume uma taxa de crescimento constante a partir daí. Os boxplots do número de espécies descritas anualmente indica um relativo baixo número de descrições no século 18, algo esperado pela recém criação do sistema lineniano. Por outro lado, eles também deixam claro a taxa alta de descrições do século 21, que tem uma mediana de aproximadamente 300 espécies descritas por ano. Embora apenas 20% do século tenha passado, ele já tem cerca de 80% do total de descrições do século 20. O ano com o maior número de descrições até hoje, de acordo com a amostra, é o ano de 2001, que registrou a descrição de 855 espécies.

Usando a coluna `desc_author` do `df_master`, é possível ver qual foi o autor mais prolífico desse ano:
...
```
O fato de o termo *Orgs.* aparecer no autor com maior número de descrições de 2001 indica ser uma obra. Ainda, o número de espécies descritas associadas com esse nome é muito maior que a média dos demais autores, o que faz sentido, dado que a obra deve compilar descrições de uma série de autores. Uma busca rápida me mostrou que um livro entitulado [“Biodiversidade de algas de ambientes continentais do Estado do Rio de Janeiro”](https://www.researchgate.net/publication/44430175_Biodiversidade_de_algas_de_ambientes_continentais_do_Estado_do_Rio_de_Janeiro_organizadoras_Mariangela_Menezes_Izabel_Cristina_Alves_Dias), com as referidas autoras. Partindo do título, podemos pressupor que uma grande quantidade de algas foram descritas nesse ano. Normalmente espera-se que o número absoluto de angiospermas descritas por ano seja maior que os demais grandes grupos, isso fica mais evidente quando os dados são agrupados por século:
...
p3 <- df_master %>% ggplot(aes(y=as.numeric(desc_year),x=major_group,fill=major_group)) +
  xlab("Major group") + 
...
Ambos gráficos contém a mesma informação, mas dado que a quantidade de angiospermas descritas é muito grande e a quantidade de gimnospermas descritas muito pequenas, colocar em escala logarítmica permite-nos visualizar este último grupo. Gimnospermas aparecem descritas apenas no século 19 e 21, sendo a maioria no primeiro. Esse fato é reforçado no gráfico de violino, que indica essa maior densidade de descrições ao longo do século 19, seguido por um hiato até o século 21, onde mais algumas poucas descrições são realizadas. Os gráficos também reforçam a importância do século 21 para a descrição de algas, que tiveram, além de uma maior quantidade em números absolutos, uma maior quantidade de espécies do total do grupo descritas recentemente. Embora, em média, uma maior quantidade de angiospermas sejam descritas anualmente, esse não foi o caso no ano de 2001, ano de lançamento do livro:
...
Em 2001 o total de algas descritas foi quase o triplo do de angiospermas. Além dessa questão, dado que a obra trata da baía de Guanabara, espera-se que isso seja refletido no total de espécies descritas para o estado neste ano. Isso pode ser visualizado em um heatmap, primeiro englobando todo intervalo de tempo de descrições da amostra:
...
Poderíamos esperar que, além da questão do financiamento, estados maiores possuem maior quantidade de espécies descritas, mas este definitivamente não é o caso, uma vez que o Rio de Janeiro, por exemplo, é um dos menores estados brasileiros. Agregando mais algumas informações isso também pode ser melhor visualizado. Além do número de espécies descritas por ano, vou combinar a extensão territorial dos estados, se eles fazem fronteiras internacionais ou não  e a proporção de espécies endêmicas do total de espécies nativas descritas em cada um. Plotando todos esses dados em um único gráfico, posso tentar visualizar algumas coisas:
...
**colocar dez formas de vida mais frequentes de outra cor**
...
Nesse gráfico estão representadas os três tipos de vegetação mais frequentes para cada forma de vida e em qual proporção. Fica logo evidente a frequência com que domina a floresta ombrófila (laranja) dentre elas. Quase todas formas de vida tem essa vegetação dominando dentre as mais frequentes, o que é minimamente esperado dado que uma grande quantidade de espécies nativas ocorre em Mata Atlântica, domínio no qual este tipo de vegetação está mais associado. Outro fato interessante evidenciado por esse gráfico é a importância das vegetações sobre afloramentos rochosos para espécies suculentas: apenas nestas este tipo de vegetação figura entre as mais frequentes. É também curioso que a Floresta Ombrófila Mista esteja aparecendo nesse gráfico, dado que trata-se de um tipo de vegetação bastante devastado e restrito a regiões específicas do sul do Brasil. Ainda, todas as formas de vida que possuem essa vegetação figurando como frequente estão associadas como briófitas, algo que merece uma investigação futura. 
...
Embora a esmagadora maioria das espécies com o nome "Envira" associado pertençam à familia Annonaceae, ele também parece associado a espécies de outras famílias. Embora não seja um resultado surpreendente, continua sendo um fato interessante.
